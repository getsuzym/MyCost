...
There is one source of truth — the app's `ModelContainer.mainContext`. Every screen reads it via `@Query`; nothing copies `Transaction` arrays between screens (`SpendingAnalytics` is a pure function fed by `DashboardView`'s `@Query`). `DashboardView` has a month stepper (`monthAnchor`, default current month) so an imported prior-month statement is visible without a manual refresh.
