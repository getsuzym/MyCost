...
5. `ReviewTransactionsView` prioritizes the fields that need attention: include/exclude, merchant, amount, **category** (picker sorted with `alphabetizedByName()`), **recurring** (a per-row `isRecurring` toggle + an optional "mark future transactions from this merchant as recurring"), and a **rule-status indicator** (`draft.categorizationStatus` → `.ruleMatched` / `.categorized` / `.uncategorized`; the last is the orange "needs a category" state). Pending/Posted was removed from the Review UI — `OCRReviewField` has no `.status` case — but `OCRTransactionDraft.status` is still kept and stored on the `Transaction` for duplicate detection. The Picker uses a custom `Binding` whose `set` calls `markCategoryUserSet` so a rule-matched row that the user overrides flips to `.categorized` (programmatic rule/suggestion assignment doesn't). It also has an **Account Type** section (one picker per distinct draft account name, seeded from any saved `Account`), a per-row deterministic **"Suggest category"** button (`MerchantCategorizationCoordinator`), plus Select All / Deselect All and "View source" (batch). `saveApprovedTransactions` first `AccountService.upsert`s the type choices, then runs `flagDuplicateDrafts` (`DuplicateMatchingService`, against existing transactions **and earlier drafts in the same batch**): **high**-confidence matches deselect their draft; **medium** ones only get a `duplicateSummary` note and are still imported, flagged `.possibleDuplicate`.
...
`MerchantRulesView` is the management screen: add/edit/delete, enable/disable (swipe), match-type picker, priority stepper, resulting merchant name + category, an "also mark matching transactions recurring" toggle + frequency, and a live "Matches / No match" preview against an example description. `learnRule(…, matchType:isRecurring:recurringFrequency:)` create-or-updates by `(caseFolded matchText, matchType)` — no duplicates. The transaction editor's "Remember merchant change?" offers **"Apply to transactions containing '<name>'"** (a `.contains` rule keyed on the new merchant name) or **"Apply to this exact merchant"** (`.exact` on the original description); a separate "mark future transactions from this merchant as recurring" toggle makes a recurring `.contains` rule. OCR-import merchant/category learning stays `.exact`.
...
The source of truth for "this month's recurring spend" is the per-transaction `isRecurring` flag; `SpendingAnalytics.recurringTotal` / `nonRecurringTotal` split on it. `RecurringPaymentsView` has an **"Recurring This Month"** section: a month stepper, every `isRecurring` transaction in that month (any category) with merchant / amount / category / date / frequency, the **total** recurring spend and the **count**, each row a `NavigationLink` to `TransactionDetailView` (edit or toggle off). `RecurringPaymentSuggestionService` still auto-*suggests* series (groups posted non-excluded transactions by account + normalized merchant key ≥ 2, infers frequency, rejects irregular cadence / high variance) and powers the expected-monthly figure, but marking recurring never requires a suggestion or a "Subscription" category.
